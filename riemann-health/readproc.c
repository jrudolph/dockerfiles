...
int read_cmdline(char *restrict const dst, unsigned sz, unsigned pid) {
    char path[PROCPATHLEN];
    snprintf(path, sizeof(path), "/host/proc/%u", pid);
...
static int simple_nextpid(PROCTAB *restrict const PT, proc_t *restrict const p) {
  ...
  p->tid = p->tgid;
  memcpy(path, "/host/proc/", 11);
  strcpy(path+11, ent->d_name);  // trust /proc to not contain evil top-level entries
  return 1;
}
...
static int simple_nexttid(PROCTAB *restrict const PT, const proc_t *restrict const p, proc_t *restrict const t, char *restrict const path) {
  ...
  if(PT->taskdir_user != p->tgid){
    ...
    snprintf(path, PROCPATHLEN, "/host/proc/%d/task", p->tgid);
...
//t->ppid = p->ppid;  // cover for kernel behavior? we want both actually...?
  snprintf(path, PROCPATHLEN, "/host/proc/%d/task/%s", p->tgid, ent->d_name);
  return 1;
}
...
static int listed_nextpid(PROCTAB *restrict const PT, proc_t *restrict const p) {
  ...
  pid_t tgid = *(PT->pids)++;
  if(likely(tgid)){
    snprintf(path, PROCPATHLEN, "/host/proc/%d", tgid);
    p->tgid = tgid;
...
PROCTAB* openproc(int flags, ...) {
    ...
    if (!did_stat){
        task_dir_missing = stat("/host/proc/self/task", &sbuf);
        did_stat = 1;
    }
    PT->taskdir = NULL;
    PT->taskdir_user = -1;
...
void look_up_our_self(proc_t *p) {
    ...
    if(file2str("/host/proc/self", "stat", &ub) == -1){
